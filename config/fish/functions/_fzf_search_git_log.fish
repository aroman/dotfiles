function _fzf_search_git_log --description "Search the output of git log and preview commits. Replace the current token with the selected commit hash."
    if not git rev-parse --git-dir >/dev/null 2>&1
        ...
        if not set --query fzf_git_log_format
            ...
            set -f fzf_git_log_format '%C(bold blue)%h%C(reset) - %C(cyan)%<(14)%ar%C(reset) %C(yellow)%d%C(reset) %C(normal)%s%C(reset)  %C(dim normal)[%an]%C(reset)'
        end
        ...
        set -f selected_log_lines (
            git log --no-show-signature --color=always --format=format:$fzf_git_log_format | \
            _fzf_wrapper --ansi \
                --multi \
                --scheme=history \
                --prompt=" " \
